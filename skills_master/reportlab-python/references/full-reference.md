...
description: >
  ReportLab — Python PDF generation using Canvas (low-level drawing) and Platypus (high-level document layout
  with flowables, tables, paragraph styles, and page templates). Use this skill whenever the user wants to
  create PDFs in Python using ReportLab, generate reports/invoices/certificates from Python data, build
  documents with Platypus flowables and templates, draw charts or vector graphics on a PDF canvas, create
  tables in Python PDFs, or work with paragraph styles and page layouts. Also trigger when the user mentions
  "reportlab", "platypus", "SimpleDocTemplate", or asks to generate PDFs from Python — even if they don't
  name the library explicitly. If the user is working in Python and needs to create PDFs from structured data
  (not from a webpage), this is the skill to use.
---
