...
description: Transactional email sending with Resend, React Email templates, and multi-channel notification systems. Use this skill when Resend API, React Email, email template, transactional email, SPF/DKIM/DMARC, email deliverability, BullMQ email queue, notification system, email verification, digest email, batch email pipeline, email bounce handling, multi-channel notifications. Use this skill when implementing transactional email flows, building email templates with React Email and Tailwind, setting up email authentication (SPF/DKIM/DMARC), configuring async email queues with BullMQ, designing notification preferences and unsubscribe flows, implementing multi-channel notifications (email + in-app + push), handling bounces and failed deliveries, and building batch/digest email pipelines.
---
