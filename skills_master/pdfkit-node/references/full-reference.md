...
name: pdfkit-node
description: >
  PDFKit — Node.js PDF document generation with a chainable API for text, vector graphics, images, tables,
  fonts, annotations, and accessibility features. Use this skill whenever the user wants to create PDFs
  programmatically in JavaScript or TypeScript using PDFKit, generate invoices/reports/certificates from data,
  build PDF documents with custom layouts, embed fonts and images in PDFs, create vector graphics in PDF format,
  add tables to PDF documents in Node.js, or work with PDF annotations and forms. Also trigger when the user
  needs to generate PDFs from structured data (not from a webpage — that's Puppeteer's domain), create
  printable documents, build PDF templates, or mentions "pdfkit" in any context. If the user wants a PDF
  created from code and they're in a Node.js/TypeScript context, this is the right skill.
---
