---
name: ripgrep
description: Fastest recursive text search tool for files and codebases using ripgrep (rg). Use this skill when grep, rg, ripgrep, search for text in files, find all occurrences, locate definitions, trace imports, hunt for secrets, regex pattern matching, codebase search, log analysis, dead code detection, file content scanning, find-and-replace across project, filter file types, count pattern occurrences, audit files for security issues.
---
